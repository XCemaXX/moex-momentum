...
### 2026-09-07 — проверка «сдвиг запятой» (task 046)

Детектор искал строку, которая ровно в 10× больше **обеих** соседних выплат того же
тикера — сигнатуру потерянной запятой. Дал 10 кандидатов. **Все десять оказались
настоящими выплатами**, детектор удалён (точность 0%).

| Тикер | Дата | Сумма | Вердикт | Чем подтверждено |
|---|---|---:|---|---|
| NMTP | 2015-07-13 | 0.2336 | верна | 4.499 млрд ₽ / 19 259 815 400 акций = 0.23360 ровно; совет директоров рекомендовал увеличить дивиденд-2014 **в 10 раз** |
| KTSBP | 2011-04-11 | 0.364239 | верна | 1prime + investfunds за 2010 г.; обе доли получили поровну |
| GAZC | 2025-12-27 | 12.5632 | верна | investfunds: 9 мес 2025, отсечка 27.12.2025 |
| GAZC | 2021-12-26 | 3.38 | верна | investfunds: 9 мес 2021; близнец GAZS в тот же день 3.22 |
| SIBN | 2015-10-16 | 5.92 | верна | ВОСА 30.09.2015 утвердило 5.92 ₽ за 1П2015 |
| SFIN | 2025-12-25 | 902.00 | верна | распределение денег от продажи Европлана: 43.9 млрд ₽ / 48 622 391 акц. ≈ 903 |
| NAUK | 2025-05-25 | 26.76 | верна | акционеры утвердили 26.76 ₽ за 2024 г. |
| LNZLP | 2013-08-21 | 433.72 | верна | T-Bank: 21.08.2013 = 433.72 (доходность 21.5%); не пересчёт сплита |
| GAZS | 2022-12-25 | 8.05 | верна | smart-lab: 3кв 2022; близнец GAZC 8.42 |
| TORS | 2016-07-13 | 0.0962 | верна | smart-lab + investmint, обе доли, за 2015 г. |

**Почему детектор промахнулся.** Он предполагал одну каденцию выплат на бумагу.
GAZC и GAZS (это **ГАЗКОН** и **ГАЗ-сервис**, не газораспределение) платят крупный
девятимесячный аванс в декабре и символический итог в июле — отношение структурно
десятикратное. SIBN так же: 5.92 промежуточный, 0.55 итоговый. Остальное — настоящие
спецвыплаты и бумаги с рваной историей.

**Побочно: `NMTP 2011-05-11` перепроверен.** T-Bank до сих пор отдаёт 0.23 —
неисправленное десятикратное значение. Против него: dohod даёт 0.023, investmint даёт
ровный ряд ~0.023, и арифметика — 0.23 это ~4.43 млрд ₽, почти вся прибыль 2010 года у
компании, только что набравшей долг под покупку ПТП. **Правка `task 034` верна, 0.023
оставить.** T-Bank — источник с ошибкой на этой бумаге.

**Открытый вопрос: `LNZLP` 2010.** В файле `2010-05-20 = 98.5` (tbank) и
`2010-08-09 = 98.5` (yahoo) — одна сумма от двух источников с разницей 81 день.
Существующий дедуп смотрит окно 7 дней и такое не видит. Похоже на одну выплату,
записанную дважды; тогда 2010 год завышен вдвое. Подтвердить не удалось — таблицы всех
провайдеров по Лензолоту начинаются с 2012 года. Оставлено как есть.

**Замечание по датам yahoo.** У TORS и KTSBP `registry_close` из `skill_fill_yahoo`
смещён на 1–3 дня от настоящей отсечки: это ex-date, что и записано в
`registry_close_source=yahoo_ex_div`. На суммы не влияет.
