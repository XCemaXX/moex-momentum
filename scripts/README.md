...
| `backfill/` | done | Artefact of the completed one-time dividend backfill: `fetch_yahoo`/`fetch_tbank` populated `.fill_cache/`, `cascade_merge` folded the result into `data/dividends/`. Still runnable cache-only if a re-fill is ever needed. |
