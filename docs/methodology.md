...
- **Дивиденды не в рублях — не учитываются:** конвертация валют вне скоупа, записи с `currency ≠ RUB` отбрасываются. Затронуты INGR, POLY, RUAL, T. В месяцы таких выплат total return бумаги занижен — смещение одностороннее.
